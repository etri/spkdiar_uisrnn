...
def parse_arguments():
  ...
  model_parser.add_argument(
      '--model_name',
      default='saved_model.uisrnn',
      help='UIS-RNN model name')
  ...
  training_parser.add_argument(
      '--train_sequences',
      default='',
      help='Train sequences')
  training_parser.add_argument(
      '--train_cluster_ids',
      default='',
      help='Train cluster ids')
  training_parser.add_argument(
      '--uisrnn_log_file',
      default='./train.log',
      help='UISRNN log filename')
  ...
  inference_parser.add_argument(
      '--test_cluster_ids',
      default='',
      help='Test cluster ids')
  inference_parser.add_argument(
      '--delay',
      default=0,
      type=int,
      help='The number of delay steps during inference.')

  # dvector extractor configurations
  dvector_parser = argparse.ArgumentParser(description='dvector extractor configurations.', add_help=False)
  dvector_parser.add_argument('--checkpoint', type=str, required=True, help='Directory of checkpoint')
  dvector_parser.add_argument("--gpu", type=int, default=-1,
                               help="GPU-id to offload model to, -1 means running on CPU")
  dvector_parser.add_argument("--dwin", type=int, default=24, help="The number of frames of dvector window")
  dvector_parser.add_argument("--dhop", type=int, default=12, help="The number of frames of dvector hop")
  dvector_parser.add_argument("--wav_file", type=str, required=True, help="wav file")
  dvector_parser.add_argument("--vad_file", type=str, required=True, help="vad file")
  dvector_parser.add_argument("--out_file", type=str, required=True, help="output file")
  dvector_parser.add_argument("--dvector_log_file", type=str, default='./dvector4uisrnn.log',
                               help="dvector log filename")

  # a super parser for sanity checks
  super_parser = argparse.ArgumentParser(
      parents=[dvector_parser, model_parser, training_parser, inference_parser])

  # get arguments
  super_parser.parse_args()
  dvector_args, _ = dvector_parser.parse_known_args()
  model_args, _ = model_parser.parse_known_args()
  training_args, _ = training_parser.parse_known_args()
  inference_args, _ = inference_parser.parse_known_args()

  return (dvector_args, model_args, training_args, inference_args)
