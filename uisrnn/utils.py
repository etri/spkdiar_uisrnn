...
def output_result(model, model_args, training_args, test_record):
  """Produce a string to summarize the experiment."""
  accuracy_array, ground_truth_array, predicted_id_array = zip(*test_record)
...
Performance:
  ...
  """.strip().format(
      ...
      total_accuracy)
  for accuracy, ground_truth, predicted_id in zip(accuracy_array, ground_truth_array, predicted_id_array):
    output_string += '\n    {:.6f} {:2d} {:2d}'.format(accuracy, ground_truth, predicted_id)
  output_string += '\n' + '=' * 80 + '\n'
